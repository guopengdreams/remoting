...
public class PerformanceClientTest extends TestCase {
    ...
    public void testClient() throws Throwable {
        ...
        URL url =
                URL.valueOf("netty4://192.168.2.234").setPort(9911)
        ...
        List<String> serverEnvironment =
                (List<String>) nettyClients[0].request("environment").get();
                    ...
                    try {
                        ...
                        for (int i = 0; i < runs; i++) {
                            try {
                                ...
                                NettyClient client =
                                        nettyClients[index.getAndIncrement() % connections];
            ...
            public void run() {
                try {
                    ...
                    while (latch.getCount() > 0) {
                        ...
                        if (!bfirst) // 第一次不准
                            System.out.println("[" + dateFormat.format(new Date()) + "] count: "
                                    + count.get() + ", error: " + error.get() + ",tps:"
                                    + (c / elapsd));
